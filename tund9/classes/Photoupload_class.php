	...
	class Photoupload{
		...
		public function addWatermark($wmfile){
			if(isset($this->mynewtempimage)){
				...
				//kopeerime vesimärgivähendatud pildile
		...
		public function moveorigimage($inputerror, $filename, $photouploaddir_orig){
			$notice = null;
			$this->filename = $filename;
			$this->photouploaddir_orig = $photouploaddir_orig;
			$this->inputerror = $inputerror;
			if(empty($this->inputerror = $inputerror)){
				if(move_uploaded_file($this->photoinput["tmp_name"], $this->photouploaddir_orig .$this->filename)){
					...
					$this->inputerror = $inputerror .= " Originaalfaili üleslaadimisel tekkis tõrge!";
		...
		public function filesizecheck($filesizelimit, $inputerror){
		...
		public function doesfileexist($filename, $photouploaddir_orig){
			$this->filename = $filename;
			$this->photouploaddir_orig = $photouploaddir_orig;
			if(file_exists($this->photouploaddir_orig .$this->filename = $filename)){
				$inputerror = "Selle nimega fail on juba olemas!";
				return $inputerror;
			}
		}
		public function filenamecreation($filenameprefix, $timestamp){
			$this->filenameprefix = $filenameprefix;
			$this->timestamp = $timestamp;
			$filename = $this->filenameprefix .$this->timestamp ."." .$this->photofiletype;
		}
